...
els.symbolsInput.addEventListener("change", () => {
  draw();
  schedulePersistWork();
});
els.gridInput.addEventListener("change", () => {
  draw();
  schedulePersistWork();
});
els.onlyColorInput.addEventListener("change", () => {
  draw();
  schedulePersistWork();
});
...
function setStitched(index, value) {
  const before = stitched.has(index);
  if (value) stitched.add(index);
  else stitched.delete(index);
  if (before !== stitched.has(index)) schedulePersistWork();
}
...
function exportWork() {
  if (!project) return;
  const json = JSON.stringify(buildViewerPayload(), null, 2);
  persistWork(json);
...
function schedulePersistWork() {
  if (!project || !window.AndroidBridge || typeof window.AndroidBridge.persistViewerState !== "function") return;
  clearTimeout(persistTimer);
  persistTimer = setTimeout(persistWork, 250);
}

function persistWork(json) {
  if (!project || !window.AndroidBridge || typeof window.AndroidBridge.persistViewerState !== "function") return;
  try {
    window.AndroidBridge.persistViewerState(json || JSON.stringify(buildViewerPayload(), null, 2));
  } catch (error) {
    console.error(error);
  }
}
